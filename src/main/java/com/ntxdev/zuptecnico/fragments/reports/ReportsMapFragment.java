 ...
 */
public class ReportsMapFragment extends Fragment implements GoogleMap.OnCameraChangeListener {
    ...
    public void onCameraChange(CameraPosition cameraPosition) {
        ...
        double distance = GeoUtils.distance(region.latLngBounds.northeast, region.latLngBounds.southwest);
    ...
    private void fillMap(ReportItemCollection items) {
        ...
        for(MapCluster cluster : items.clusters) {
            ...
            if(cluster.category_id != null) {
                category = Zup.getInstance().getReportCategoryService().getReportCategory(cluster.category_id);
            }
            ...
            Bitmap bmp = BitmapUtil.getMapClusterBitmap(cluster, getResources().getDisplayMetrics(), color);
    ...
    class Tasker extends AsyncTask<Void, Void, ReportItemCollection> {
        ...
        @Override
        protected ReportItemCollection doInBackground(Void... voids) {
            try {
                // Is this the final position?
                Thread.sleep(1000);
                return Zup.getInstance().getService().retrieveReportItems(latitude, longitude, distance, 100, zoom);
            }
            catch (Exception ex) {
                return null;
            }
        ...
        @Override
        protected void onPostExecute(ReportItemCollection reportItems) {
